...
|---|---|---|---|---|
| policy [F0.7](03-shell.md#f07-derive-the-editing-policy-new) | which fields the context speaks to | the field is known and in range | – | the defaults |
| describe [F1.7](04-prepare.md#f17-describe-per-footage-source-chunks-of-pmachinedescribeframesperreq--4) | what is on each frame; the running state | which frame an event landed on | folding "same" rows when the file is read | frames per request, context windows |
| fix [F1.8](04-prepare.md#f18-fix-the-transcripts-blocks-of-pmachinefixblocklines--25-lines) | the wording of a line | the line is in this block; no tab | building `session.tsv` | block size, context seconds |
...
13. **Silences over eight seconds are cut out of kept clips**, on the stated assumption the model will not → stays after `finish`; a deliberate long beat is a policy number (`P.policy.deadAirMaxSeconds`), not a silent rule. The cut brief states the number, so the model can keep a beat by asking.
...
21. **The target length is a regex over free text with no fallback**: "half an hour" silently becomes no target, changing the prompt, both count gates and the length gate at once → `P.policy.targetLengthSeconds`, derived by [F0.7](03-shell.md#f07-derive-the-editing-policy-new)'s `set_policy`, shown in the policy form.
...
23. **Placement is overridden three ways at render time**: an overrunning line pushes every following line later, the whole schedule slides earlier, and speech is sped to P.eng.narrationMaxTempo — so the **last** line on a clip falls off, usually the punchline the prompt asked to land → `write_line`'s **result** says where the line lands and whether it will be squeezed; `finish` names clips that will not fit, so they can be rewritten shorter. The page already warns the user; only the model is kept in the dark.
24. **The hand-curated "this clip plays its own audio" list is wiped on every ▶** — the user's decisions destroyed by the next narration run → fixed in the rewrite ([`07-narrate.md`](07-narrate.md) [F4.1](07-narrate.md#f41--write-and-speak)): the silent list is the user's, not the model's, and survives a rewrite of the lines.
...
34. **The title is printed onto the picture only if it was the first title to exist**; later rewrites leave the picture's words alone → the rewrite reprints the title whenever the text changes, until the picture is edited by hand ([`08-produce.md`](08-produce.md) [F5.6](08-produce.md#f56-upload-text-and-thumbnail)).
...
Four prototype behaviours are marked defects, not designs: the narration run wiping the user's silent list (§3.24, **MUST fix**), the subtitle placement tag travelling through the translation as a word (§3.37, **MUST fix**), the thumbnail title printed only once (§3.34, decided in F5.6), and the three disagreeing readings of a narration line's `at` on a sped-up clip (decided in [`01-project-and-files.md` §4](01-project-and-files.md#4-narratenarrationjson): the recording's own clock).

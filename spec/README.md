...
- **Screens**: ASCII mockups. Layout indicative; widgets, their order, labels and tooltips normative.
...
- **Every flow is drawn and written**: under each `F<n>.<m>` heading, a diagram (branches, refusals, the dialog, or the timeline before/after), then the numbered steps. Where they disagree, the steps are normative.
...
- **Strings in quotes** ("…"): the prototype's user-visible text; keep unless the review changes it. Log lines start `>>> ` (progress), `!!! ` (failure) or four spaces (detail).
